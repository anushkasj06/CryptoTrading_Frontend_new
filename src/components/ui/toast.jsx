...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-2 overflow-hidden rounded-xl border p-4 pr-6 shadow-2xl transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full",
  {
    variants: {
      variant: {
        default: "border-slate-700 bg-gradient-to-br from-slate-900 via-slate-950 to-black text-slate-50 border-l-4 border-l-slate-500",
        success:
          "border-emerald-500/50 bg-gradient-to-br from-emerald-950/95 via-emerald-900/95 to-emerald-950/95 text-emerald-50 border-l-4 border-l-emerald-500 shadow-emerald-500/20",
        destructive:
          "destructive group border-rose-500/50 bg-gradient-to-br from-rose-950/95 via-rose-900/95 to-rose-950/95 text-rose-50 border-l-4 border-l-rose-500 shadow-rose-500/20",
      },
    },
    defaultVariants: {
      variant: "default",
...
const ToastDescription = React.forwardRef(({ className, ...props }, ref) => (
  <ToastPrimitives.Description ref={ref} className={cn("text-sm opacity-95 font-medium", className)} {...props} />
))
